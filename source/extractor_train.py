...
def train(args, train_dataset, model, tokenizer, eval_dataset=None, eval_fn=None):
    ...
    # Select a random sampler based on whether distributed training is used.
    # Verify that the training batch size is
    # compatible with the number of GPUs and the number of gradient accumulation steps.
    # Initialise the data loader train_dataloader.
    ...
    for _idx_epoch, _ in enumerate(train_iterator):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                # save checkpoint
                if args.save_checkpoint and args.local_rank in [-1,
                                                                0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    save_model_with_default_name(args.output_dir + '/checkpoints/' + str(global_step), model, tokenizer,
                                                 args)
...
def evaluation(args, eval_dataset, model, tokenizer, global_step=None, mode='eval', dataset_type=None):
    ...
    for batch in tqdm(eval_dataloader, desc='Evaluating'):
        ...
        for _i in range(len(batch[1])):
            ...
            if args.get_evidences:
                ...
                zipped_true_evidence_details = zip(true_evidence_text, sent_true_scores, sent_true_labels.tolist(),
                                                   sent_true_idx.tolist())
                true_evidence_details = [[a, b, c, d] for a, b, c, d in zipped_true_evidence_details]

                zipped_false_evidence_details = zip(false_evidence_text, sent_false_scores, sent_false_labels.tolist(),
                ...
                tmp_evidence_dict[batch[1][_i]['event_id']]['predicted_score'] = ["{:.3f}".format(e) for e in list(
                    predict_scores[_i].detach().cpu().numpy())]
                tmp_evidence_dict[batch[1][_i]['event_id']]['logsoftmax_predicted_score'] = ["{:.3f}".format(e) for e in
                                                                                             list(torch.softmax(
                                                                                                 predict_scores[_i],
                ...
                tmp_evidence_dict[batch[1][_i]['event_id']]['num_overlap'] = [len(sent_true_idx), len(sent_false_idx),
                                                                              len(set(sent_true_idx) | set(
                                                                                  sent_false_idx))]
    ...
    if args.get_evidences:
        ...
        with open(os.path.join(args.output_dir, dataset_type + '_' + str(args.top_k) + '_evidence_details.json'),
                  'w') as f:
...
def main():
    ...
    train_dataset = NewsDataset(args.dataset_name, train_raw_dataset, tokenizer, args.max_seq_length,
