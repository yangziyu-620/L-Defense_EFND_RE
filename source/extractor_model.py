...
class EvidenceSelection(RobertaPreTrainedModel):
    ...
    def get_correlation_scores(self, claim_embds, sents_embds):
        ...
        if self.correlation_method == 'mlp':
            concat_embds = torch.cat(
                [claim_embds * sents_embds, claim_embds - sents_embds, claim_embds.repeat(sents_embds.size(0), 1),
                 sents_embds], dim=-1)
    ...
    def forward(self, claim_ids, claim_mask_ids, claim_labels,
        ...
        valid_sent_mask_ids = torch.concat(
            [sents_mask_ids[_i, :num_sents] for _i, num_sents in enumerate(batch_num_sents)])
        ...
        for _i in range(bs):
            sample_corre = self.get_correlation_scores(claim_embds[_i],
                                                       sents_embds[segment_indexes[_i]:segment_indexes[_i + 1]]).view(
                -1, 2)  # [batch_num_sents]
            sample_true_attn = self.true_attn(claim_embds[_i], sents_embds[segment_indexes[_i]:segment_indexes[
                _i + 1]])  # [batch_num_sents]
            sample_false_attn = self.false_attn(claim_embds[_i], sents_embds[segment_indexes[_i]:segment_indexes[
                _i + 1]])  # [batch_num_sents]
        ...
        predict_scores = torch.cat([sample_false_attned_probs.unsqueeze(1), sample_true_attned_probs.unsqueeze(1)],
                                   dim=1)
